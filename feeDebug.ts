...
const FEE_WALLET = '5GDW2fWXKP71v1WsHmvoqbNtfsQMV4m3RdtGXN5DKfJ3'; // Degen Ghost
const FEE_BPS = 100; // 1% fee

// Validate fee wallet address
try {
    new PublicKey(FEE_WALLET);
    console.log(`Fee wallet validated: ${FEE_WALLET}`);
} catch (e) {
    console.error('Invalid fee wallet address:', FEE_WALLET);
    process.exit(1);
}
...
const getQuote = async (inputMint: string, outputMint: string, amount: number) => {
    const url = `${JUPITER_QUOTE_API}?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amount}&slippageBps=${SLIPPAGE_BPS}&platformFeeBps=${FEE_BPS}&feeAccount=${FEE_WALLET}`;
    ...
    if (quote.platformFee) {
        console.log(`Platform fee included: ${quote.platformFee.amount} units to ${quote.platformFee.feeAccount}`);
    } else {
        console.log('No platform fee in quote response');
    }
    return quote;
};
